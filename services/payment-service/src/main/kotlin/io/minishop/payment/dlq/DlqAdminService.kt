...
/**
 * payment-service DLQ admin 의 *얇은* 오케스트레이션. ADR-026 의 표준 service 1 종.
 *
 * payment 특유 처리:
 * - bulk-discard 가 PAYMENT_REFUND source 라면 추가 강력 audit (돈 *돌려주는* 동작의 dead-letter
 *   를 묻는 것이 비즈니스에 어떤 영향인지 reviewer 가 더 신중하게 결정하도록 표시).
 *   본 단계는 audit 로그에 `risk=high` 표식만 추가.
 * - replay 의 idempotencyKey 는 *원 메시지의 헤더* 에서 복사 (use case 책임). 같은 PG 결제로 매핑.
 */
@Service
        ...
        audit.log(
            DlqAuditLog.AuditEvent(
                ...
                extra = buildMap {
                    ...
                    response.idempotencyKey?.let { put("pgIdempotencyKey", it) }
                    if (message?.source == DlqSource.PAYMENT_REFUND) put("risk", "high")
                },
            )
        )
        return response
    ...
    fun discard(messageId: String, actor: String, reason: String): DlqDiscardResponse {
        ...
        audit.log(
            DlqAuditLog.AuditEvent(
                ...
                extra = if (message?.source == DlqSource.PAYMENT_REFUND) mapOf("risk" to "high") else emptyMap(),
            )
        )
        return response
    }
    ...
    fun bulkReplay(request: DlqBulkRequest, actor: String): DlqBulkJobResponse {
        ...
        audit.log(
            DlqAuditLog.AuditEvent(
                ...
                extra = buildMap {
                    put("jobId", job.jobId); put("dryRun", dryRun.toString())
                    if (request.source == DlqSource.PAYMENT_REFUND) put("risk", "high")
                },
            )
        )
        return job
