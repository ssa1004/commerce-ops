...
class PaymentEventConsumer(
    private val repository: PaymentInboxRepository,
    ...
    /**
     * payment.events consume → inbox 저장. [InventoryEventConsumer] 의 같은 패턴 주석 참고
     * (트랜잭션 커밋 후 ack 명시).
     */
    ...
    fun onMessage(payload: String, acknowledgment: Acknowledgment) {
        val event: InboundPaymentEvent = try {
            objectMapper.readValue(payload, InboundPaymentEvent::class.java)
        } catch (e: Exception) {
            log.error("Cannot parse payment event payload, skipping. raw={}", payload, e)
            meterRegistry.counter(
                "inbox.consume",
                Tags.of("topic", "payment.events", "outcome", "parse_error"),
            ).increment()
    ...
    private fun persistInbox(event: InboundPaymentEvent, payload: String) {
        ...
        if (repository.existsByPaymentId(event.paymentId!!)) {
            meterRegistry.counter(
                "inbox.consume",
                Tags.of("topic", "payment.events", "outcome", "duplicate"),
            ).increment()
            return
        }
        ...
        meterRegistry.counter(
            "inbox.consume",
            Tags.of("topic", "payment.events", "outcome", "stored"),
        ).increment()
        log.debug(
            "Stored payment.events: type={} order={} payment={}",
            event.type, event.orderId, event.paymentId,
        )
    }
