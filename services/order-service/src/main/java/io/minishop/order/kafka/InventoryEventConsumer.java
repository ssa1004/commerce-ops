...
public class InventoryEventConsumer {
    ...
    public InventoryEventConsumer(InventoryInboxRepository repository, ObjectMapper objectMapper,
     ...
     * <p>처리 순서:
     * <ol>
     *   <li>parse — 실패 시 즉시 ack (DLT 로 보낼 가치 없음, 그냥 흡수). poison pill 무한 루프 방지.</li>
     *   <li>{@link TransactionTemplate#execute} 안에서 inbox 저장. 정상 return 하면 트랜잭션 커밋 완료
     *       — 여기서 throw 하면 spring-kafka 의 DefaultErrorHandler 가 retry / DLT 처리 (ack 안 함).</li>
     *   <li>트랜잭션이 무사히 커밋된 *후* {@code acknowledgment.acknowledge()} — 트랜잭션 ↔ ack 의
     *       순서 보장. 트랜잭션이 롤백되면 ack 안 일어나 같은 메시지가 다시 오고, inbox 의 UNIQUE
     *       제약이 중복을 흡수 (멱등성).</li>
     * </ol>
     *
     * <p>이전 패턴 (메서드에 {@code @Transactional} + ack-mode=record) 도 spring-kafka 의 timing
    ...
    public void onMessage(String payload, Acknowledgment acknowledgment) {
        ...
        InboundInventoryEvent finalEvent = event;
    ...
    private void persistInbox(InboundInventoryEvent event, String payload) {
