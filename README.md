...
[![Coverage: Kover](https://img.shields.io/badge/coverage-Kover-7F52FF.svg)](https://github.com/ssa1004/commerce-ops/actions/workflows/ci.yml)
...
3 서비스 모두 [Kover](https://github.com/Kotlin/kotlinx-kover) (Kotlin 코드 커버리지 플러그인) 를 적용했습니다. 프로덕션 코드는 Kotlin, 테스트는 Java/Kotlin 혼재라 둘 다 계측됩니다. Spring bootstrap / DTO / 설정 클래스는 단위 테스트 대상이 아니라 분모에서 제외해 도메인·서비스 로직의 실커버리지만 봅니다.

```bash
cd services/order-service && ./gradlew koverLog        # 라인 커버리지 % 를 콘솔에 출력
                              ./gradlew koverHtmlReport # build/reports/kover/html/index.html
                              ./gradlew koverXmlReport  # build/reports/kover/report.xml
```

CI 의 `services` 잡이 매 빌드에서 `koverLog` + XML/HTML 리포트를 생성하고, HTML 리포트를 `coverage-<service>` 아티팩트로 업로드합니다 (상단 [![Coverage: Kover](https://img.shields.io/badge/coverage-Kover-7F52FF.svg)](https://github.com/ssa1004/commerce-ops/actions/workflows/ci.yml) 배지 → CI run → Artifacts). 커버리지 게이트(`koverVerify`)는 아직 강제하지 않습니다 — 숫자를 박제하기보다 추세를 리뷰에서 본다.
