...
order-service 의 adaptive concurrency limiter 가 minLimit 근처까지 떨어졌거나 거절율이 임계 초과일 때.
...
> adaptive limiter 가 cascade 차단으로 호출을 즉시 거절하고 있다는 뜻. 이건 의도한 동작 — backend 가 무너지는 걸 막고 있는 중. 그러나 사용자 경험 (5% 가 503) 은 분명 손실 — 원인을 봐야 함.

## Impact

- **단기**: 사용자가 503 + Retry-After:1 응답을 받음. 클라이언트 재시도 로직이 있으면 1s 후 재시도, 그 사이 limiter 가 회복되어 있을 수 있음.
- **중기**: limit 이 minLimit 까지 떨어지면 대부분의 요청이 거절됨 — 사실상 partial outage. 그러나 backend 는 보호받는 중 (이걸 안 했다면 backend 도 같이 무너짐).
- **장기**: 왜 backend 가 느려졌는지가 답해지지 않으면 limit 이 항상 좁혀진 상태로 유지 → 정상 트래픽까지 거절.
...
2. **upstream 의 응답 시간** — 정말로 느려졌는지 확인:
   ...
   ```
   p99 가 평소의 5x 이상이면 limit 축소가 적절한 반응 — backend 보호 작동 중.
...
- 운영 사고 시 즉시 롤백: `mini-shop.concurrency.enabled=false` — limiter 끄고 기존 timeout-only 동작으로. cascade 위험이 다시 생기므로 backend 의존 부담을 보면서 결정.

### 단기 (1시간 안)

- backend 의 응답 시간이 평소대로 돌아왔는데 limit 만 좁혀진 채 머물면 — Gradient2 의 `smoothing` 파라미터 조정 (기본은 라이브러리 default). limit 회복 속도가 빨라짐.
- backend 의 hikari pool 을 키움 (예: 10 → 20) — 동시 처리 능력 자체를 늘려 limit 축소가 일어나지 않게.

### 장기 (PR 단위)

- backend 가 왜 느렸는지를 case-study 로. JFR chunk 분석 + slow-query / GC / 외부 의존 신호 종합.
...
- limiter 가 backend 를 얼마나 보호했는지 — limiter 가 없었다면 backend 도 같이 무너졌을 가능성 평가
- min-limit / 알람 임계 조정이 필요했는지

## 관련

- ADR-016 — Adaptive concurrency limiter (이 결정의 배경)
