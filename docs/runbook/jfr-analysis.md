...
> 알람 runbook 이 아니라 *알람을 받고 JFR chunk 를 분석해야 할 때* 의 작업 가이드.

## 언제 JFR 분석을 시작하나

- `jvm_gc_pause_too_long`, `order_p99_latency_high`, `hikari_pool_saturation` 등 *runtime 원인 의심* 알람이 떴을 때.
...
`cpu-flame.html` 을 브라우저로 열면 각 stack frame 의 CPU 사용률을 누적 막대로 본다. 가장 *넓은 박스* 가 핫 패스.
...
CI 에서 chunk 를 받아 *상위 10개 핫 메서드* 를 GitHub Issue 코멘트로 자동 게시 — 이 패턴은 NHN APM / 라인 LINE Profiler 가 내부 도구로 운영.
...
- Allocation 페이지의 *증가만 하는* 클래스 + Old Gen 점유 시간이 같이 늘면 누수.
...
- 다른 runbook: `gc-pause-too-long`, `order-p99-latency-high`, `hikari-pool-saturation` 의 Diagnosis 섹션이 모두 *JFR 분석으로 점프* 한다.
