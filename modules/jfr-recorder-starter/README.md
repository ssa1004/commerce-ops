...
2. 별도 스케줄러가 `rollover` (기본 5분) 주기마다:
   ...
   - 새 `Recording` 시작.
   - `maxRetained` (기본 24개 = 2시간) 초과한 오래된 chunk 삭제. 추가로 `maxTotalSize` (기본 500MB) 초과 시 가장 오래된 것부터 추가 삭제 (디스크 풀 안전망 — ADR-024).
...
mini-shop:
  jfr:
    ...
    max-total-size: 500MB         # 보존 chunk 의 총 크기 상한 (카운트 cap 다음에 추가 적용)
